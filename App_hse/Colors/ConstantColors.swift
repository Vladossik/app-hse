//
//  ConstantColors.swift
//  App_hse
//
//  Created by Vladislava on 31/01/2019.
//  Copyright © 2019 VladislavaVakulenko. All rights reserved.
//

import Foundation
import UIKit

struct Colors {
    
    static let peacockBlue = UIColor(red: 1/255.0, green: 90/255.0, blue: 171/255.0, alpha: 1)
    static let lightPeacockBlue = UIColor(red: 1.0 / 255.0, green: 90.0 / 255.0, blue: 171.0 / 255.0, alpha: 0.6)
    static let waterBlue66 = UIColor(red: 16.0 / 255.0, green: 129.0 / 255.0, blue: 233.0 / 255.0, alpha: 0.66)
    static let niceBlue86 = UIColor(red: 20/255.0, green: 101/255.0, blue: 192/255.0, alpha: 0.86)
    static let categoryItem = UIColor(red: 1.0 / 255.0, green: 90.0 / 255.0, blue: 171.0 / 255.0, alpha: 0.93)
}
